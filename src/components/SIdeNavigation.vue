<template>
	<!-- 侧边导航 -->
	<div class="suspension_btn" v-show="isShow">
		<div class="sus_item" @mouseenter="susMouseLeave(0)" @mouseleave="susMouseLeave(0, 1)">
			<div class="icon1 j_c_a_i" v-if="susdata[0].img"><img src="../assets/icon_7.png" alt /></div>
			<div class="lend j_c_a_i" v-if="susdata[0].cur" @click="switchoffchange(1)">
				<img src="@/assets/lend_img.png" alt />
				我要出借
			</div>
		</div>
		<div class="sus_item" @mouseenter="susMouseLeave(1)" @mouseleave="susMouseLeave(1, 1)">
			...
			<div class="lend j_c_a_i" v-if="susdata[1].cur" @click="switchoffchange(2)">
...
export default {
	...
	methods: {
		...
		// 显示弹框
		switchoffchange(index){
			if(index == 1){
				// 出借
				this.$emit('showLend',"1");
			}else{
				// 客服
				// window.location.href = 'https://citicskefu.citics.com/commonpc?entry_type=zjwt&ans_channel=rzrq&entranceId=rzrq&useSDKBottom=0';
