    ...
    <div>
        ...
        <div class="topButton">
            <div class="handleBtnBox">
                <el-button :disabled="canSubmitData.length==0" @click="LeftQty1Seve">自动填充展期数量（{{canSubmitData.length}}）</el-button>
                <el-upload
                    ref="upload"
                ...
                <a v-if="tableData.length === 0" href="https://statics.citics.com/xinweb/%E5%B1%95%E6%9C%9F%E6%B8%85%E5%8D%95%E5%AF%BC%E5%85%A5%E6%A8%A1%E6%9D%BF.xlsx">
        ...
        <div  class="bgfff" style="padding-bottom:30px;">
            <el-table 
                ...
                <el-table-column prop="LeftQty" align="center" label="未还数量" width="90">
                    <template slot-scope="scope">  
                        {{scope.row.LeftQty || '--'}}
                    </template>
                </el-table-column>
                ...
                <el-table-column prop="StockCode" align="center" label="证券代码" width="100">
                    <template slot-scope="scope">  
                        {{scope.row.StockCode || '--'}}
                    </template>
                </el-table-column>
                <el-table-column prop="StockName" align="center" label="证券名称" width="100">
                     <template slot-scope="scope">  
                        {{scope.row.StockName || '--'}}
                    </template>
                </el-table-column>
...
export default {
    ...
    methods: {
        ...
        showMingxi(record){
             this.detailData = {
                展期至: record.zhandate || "--",
                展期数量: record.LeftQty1 || "--",
                展期天数: record.Term || "--",
                未还数量: record.LeftQty || "--",
                约定融券费率: util.percent2(record.FeeRate),
        ...
        //确认明细
        submiZqMx(){
            var canSubmitData = this.canSubmitData
            this.successData = []
            this.failData = []
            this.$refs.uploadLoad.showModel()
            this.submitFun(canSubmitData,0,canSubmitData.length)
        },
        //提交请求
        ...
        async exportConfirm(){
            ...
            this.tableData = await this.checkImportData()
            // this.tableData = data
        ...
        checkImportData(){
            ...
            data.forEach(item => {
                ...
                if(importUtil.checkErrExtensionTerm(item.zhandate, item.Term)){
                    item.status = '导入失败（数据不符合规范）'
                }
        ...
        getContractData(item, index){
            ...
            securitiesRequest(params, 110).then((res) => { //请求成功
                ...
                if(data_.length>0){
                    ...
                    data_[0].status = '未提交'
                    this.getSingleRate(data_[0], index)
        ...
        exportData(){
            ...
            this.$nextTick(() => {
                importUtil.exportDataMethod('#tableExport', '展期');
                this.exportLoading = false;
            });
        },
    },
